...
class ChartPreview {
    ...
    updatePreview() {
        ...
        try {
            ...
            // Find all canvas elements in the chart container
            const chartContainer = document.getElementById('main-chart');
            if (chartContainer) {
                const canvases = chartContainer.querySelectorAll('canvas');
                
                // Draw each canvas onto the preview canvas
                canvases.forEach(canvas => {
                    // Skip if canvas has no dimensions or is hidden
                    if (canvas.width === 0 || canvas.height === 0 || canvas.style.display === 'none') return;
                    
                    // Draw image scaled to fit preview
                    ctx.drawImage(canvas, 0, 0, displayWidth, displayHeight);
                });
            } else if (this.sourceCanvas) {
                // Fallback to sourceCanvas if main-chart container not found (legacy behavior)
